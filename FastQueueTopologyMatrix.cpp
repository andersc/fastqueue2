...
struct Options {
    ...
    std::vector<int> selectedProducers;
    std::vector<int> selectedConsumers;
...
Options parse(int argc,char**argv) { Options o; auto parseList=[](const std::string& list, auto append){size_t start=0;while(start<=list.size()){size_t end=list.find(',',start);auto token=list.substr(start,end==std::string::npos?std::string::npos:end-start);if(token.empty()){std::cerr<<"Invalid comma-separated list\n";std::exit(2);}append(token);if(end==std::string::npos)break;start=end+1;}}; for(int i=1;i<argc;++i){std::string a=argv[i];auto v=[&]{if(++i>=argc){std::cerr<<"Missing value for "<<a<<'\n';std::exit(2);}return argv[i];}; if(a=="--output")o.output=v();else if(a=="--transfers")o.transfers=std::stoull(v());else if(a=="--rounds")o.rounds=std::stoi(v());else if(a=="--warmups")o.warmups=std::stoi(v());else if(a=="--max-cpus")o.maxCpus=std::stoi(v());else if(a=="--min-sample-ms")o.minSampleMs=std::stoi(v());else if(a=="--producer-shard")o.producerShard=std::stoi(v());else if(a=="--producer-shards")o.producerShards=std::stoi(v());else if(a=="--cpus")parseList(v(),[&](const std::string& token){o.selectedCpus.push_back(std::stoi(token));});else if(a=="--producer-cpus")parseList(v(),[&](const std::string& token){o.selectedProducers.push_back(std::stoi(token));});else if(a=="--consumer-cpus")parseList(v(),[&](const std::string& token){o.selectedConsumers.push_back(std::stoi(token));});else if(a=="--widths")parseList(v(),[&](const std::string& token){o.widths.push_back(static_cast<size_t>(std::stoul(token)));});else if(a=="--help"){std::cout<<"--output FILE --transfers N --rounds N --warmups N --max-cpus N --cpus 3,19 --producer-cpus 3,19 --consumer-cpus 0,1 --min-sample-ms N --producer-shard I --producer-shards N --widths 0,8\n";std::exit(0);}}return o; }
} // namespace
int main(int argc,char**argv) {
    auto o=parse(argc,argv);auto cpus=availableCpus(o.maxCpus);if(!o.selectedCpus.empty()){std::sort(o.selectedCpus.begin(),o.selectedCpus.end());o.selectedCpus.erase(std::unique(o.selectedCpus.begin(),o.selectedCpus.end()),o.selectedCpus.end());std::vector<CpuInfo> selected;for(const auto&cpu:cpus)if(std::binary_search(o.selectedCpus.begin(),o.selectedCpus.end(),cpu.id))selected.push_back(cpu);if(selected.size()!=o.selectedCpus.size()){std::cerr<<"Requested CPU unavailable\n";return 2;}cpus=std::move(selected);}std::vector<int> producerIds=o.selectedProducers;std::sort(producerIds.begin(),producerIds.end());producerIds.erase(std::unique(producerIds.begin(),producerIds.end()),producerIds.end());if(!producerIds.empty()){for(const int id:producerIds)if(std::none_of(cpus.begin(),cpus.end(),[&](const CpuInfo& cpu){return cpu.id==id;})){std::cerr<<"Requested producer CPU unavailable\n";return 2;}}std::vector<int> consumerIds=o.selectedConsumers;std::sort(consumerIds.begin(),consumerIds.end());consumerIds.erase(std::unique(consumerIds.begin(),consumerIds.end()),consumerIds.end());if(!consumerIds.empty()){for(const int id:consumerIds)if(std::none_of(cpus.begin(),cpus.end(),[&](const CpuInfo& cpu){return cpu.id==id;})){std::cerr<<"Requested consumer CPU unavailable\n";return 2;}}constexpr uint64_t lcm=FastQueueBatch<Item>::max_size==16?720720:840;
    ...
    uint64_t pairs=0;for(const auto&p:cpus)if((producerIds.empty()||std::binary_search(producerIds.begin(),producerIds.end(),p.id)))for(const auto&c:cpus)if(p.id!=c.id&&(consumerIds.empty()||std::binary_search(consumerIds.begin(),consumerIds.end(),c.id)))++pairs;const uint64_t total=(pairs*o.widths.size()*o.rounds+o.producerShards-1)/o.producerShards;uint64_t done=0; const auto allStart=Clock::now();
    std::cerr<<"Matrix shard "<<o.producerShard<<'/'<<o.producerShards<<": "<<cpus.size()<<" CPUs, up to "<<total<<" timed samples\n";
    for(size_t pi=0;pi<cpus.size();++pi){if(static_cast<int>(pi%o.producerShards)!=o.producerShard)continue;const auto&p=cpus[pi];if(!producerIds.empty()&&!std::binary_search(producerIds.begin(),producerIds.end(),p.id))continue;for(const auto&c:cpus)if(p.id!=c.id&&(consumerIds.empty()||std::binary_search(consumerIds.begin(),consumerIds.end(),c.id)))for(const size_t w:o.widths){uint64_t n=o.transfers;double calMs=0;if(o.minSampleMs){auto cal=dispatch(w,p.id,c.id,n);calMs=cal.millis;if(!cal.pinned&&cal.mps==0){std::cerr<<"Calibration failed producer="<<p.id<<" consumer="<<c.id<<" width="<<w<<" transfers="<<n<<'\n';return 1;}const long double want=static_cast<long double>(n)*o.minSampleMs/std::max(0.001,calMs);n=static_cast<uint64_t>((want+lcm-1)/lcm)*lcm;}for(int i=0;i<o.warmups;++i)(void)dispatch(w,p.id,c.id,n);for(int r=0;r<o.rounds;++r){auto s=dispatch(w,p.id,c.id,n);out<<p.id<<','<<c.id<<','<<p.socket<<','<<c.socket<<','<<p.core<<','<<c.core<<','<<p.smt<<','<<c.smt<<','<<w<<','<<r<<','<<(s.pinned?1:0)<<','<<std::setprecision(12)<<s.mps<<','<<n<<','<<calMs<<'\n';out.flush();++done;auto elapsed=std::chrono::duration<double>(Clock::now()-allStart).count();double eta=done?elapsed*(total-done)/done:0;std::cerr<<"progress "<<done<<'/'<<total<<" ETA "<<static_cast<uint64_t>(eta)<<"s\n";}}}
}
